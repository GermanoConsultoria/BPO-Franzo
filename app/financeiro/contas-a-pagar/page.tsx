...
export default async function ContasAPagarPage() {
  ...
  const lancamentos = await prisma.lancamentoFinanceiro.findMany({
    where: { workspace_id: usuario.workspace_id, tipo: 'DESPESA' },
    include: { plano_contas: true, anexos: true },
    orderBy: { dt_vencimento: 'asc' },
  })

  const planoContas = await prisma.planoContas.findMany({
    where: { workspace_id: usuario.workspace_id, tipo: 'DESPESA', ativo: true },
    orderBy: { nome: 'asc' },
  })
  ...
  return (
    <div className="p-4 lg:p-8 max-w-7xl mx-auto">
      <header className="mb-6 border-b border-border pb-4">
        ...
        <p className="text-sm text-gray-500 mt-1">Gerencie suas despesas e obrigações financeiras.</p>
      </header>
      <LancamentosView
