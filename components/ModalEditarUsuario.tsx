  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface rounded-xl shadow-2xl w-full max-w-md p-6 border border-border animate-in fade-in zoom-in duration-200">
        <h2 className="text-xl font-bold text-foreground mb-4">Editar Usuário</h2>
        ...
        <form action={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input name="nome" defaultValue={usuario.nome} required className="w-full bg-surface-highlight border border-border rounded-lg px-3 py-2 text-foreground focus:ring-2 focus:ring-indigo-500 outline-none" />
          </div>

          <div>
            <label className="block text-xs font-semibold text-text-muted uppercase mb-1">E-mail de Acesso</label>
            <input name="email" type="email" defaultValue={usuario.email} required className="w-full bg-surface-highlight border border-border rounded-lg px-3 py-2 text-foreground focus:ring-2 focus:ring-indigo-500 outline-none" />
          </div>
              ...
              <div className="space-y-2">
                {Object.entries(PERMISSOES_LABEL).map(([chave, label]) => (
                  <label key={chave} className="flex items-center gap-2 text-sm text-foreground">
                    <input type="checkbox" name="permissoes" value={chave} defaultChecked={permissoesAtuais.includes(chave)} className="accent-indigo-600" />
                    {label}
                  </label>
                ))}
              </div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-text-muted hover:text-foreground transition-colors">Cancelar</button>
            <button
              type="submit"
              disabled={isLoading}
